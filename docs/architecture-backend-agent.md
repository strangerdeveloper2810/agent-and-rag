...
> **Lưu ý (2026-08):** Tài liệu này mô tả kiến trúc LangGraph/LangChain — vẫn giữ nguyên, chạy được qua `AGENT_BACKEND=langgraph`. Đường chạy chính hiện tại (production) là service Go riêng `services/agent-go` — xem kiến trúc agent-go + BFF (Fastify là gateway/proxy, không còn chạy agent in-process) tại [`docs/ARCHITECTURE_DEEP_DIVE/agent-go-and-bff/`](./ARCHITECTURE_DEEP_DIVE/agent-go-and-bff/README.md).
