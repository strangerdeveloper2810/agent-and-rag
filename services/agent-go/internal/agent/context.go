...
func BuildSystemPrompt(memories []string, skillSummaries []skills.SkillSummary, lang string) string {
	...
	b.WriteString("  + Khi người dùng đưa ra ý tưởng, yêu cầu mới, hoặc cần tư vấn kiến trúc/kế hoạch: TUYỆT ĐỐI KHÔNG đoán mò hoặc vội vàng kết luận.\n")
	...
	b.WriteString("    * `prompt`: Nội dung câu hỏi cụ thể, đi thẳng vào vấn đề kỹ thuật/nghiệp vụ then chốt (ví dụ: 'Kiến trúc backend và cơ sở dữ liệu chính bạn muốn sử dụng?').\n")
	b.WriteString("    * `header`: Tiêu đề ngắn gọn cho nhóm câu hỏi (ví dụ: 'Backend & DB', 'Auth & Security', 'Deployment').\n")
	...
	b.WriteString("    * `multi_select`: true nếu câu hỏi cho phép kết hợp nhiều thành phần (ví dụ: các tính năng cần có trong MVP).\n")
