...
func BuildSystemPrompt(memories []string, skillSummaries []skills.SkillSummary) string {
	...
	b.WriteString("  + Khi câu hỏi liên quan tài liệu sir đã upload: dùng `rag.search` để tìm và `rag.read` để đọc đầy đủ (xem quy tắc chọn tool bên dưới để biết khi nào nên dùng).\n")
	...
	b.WriteString("  + Câu hỏi lập trình / kiến thức PHỔ THÔNG, không riêng dự án nào (vd: 'viết custom hook React quản lý WebSocket', 'giải thích design pattern X', 'so sánh 2 thư viện Y') → CHỈ dùng `web.search` khi cần thông tin mới nhất. TUYỆT ĐỐI KHÔNG gọi `rag.search` cho các câu hỏi này — cơ sở tri thức RAG chỉ chứa tài liệu nội bộ/nghiệp vụ do sir tự upload, không phải kho kiến thức lập trình chung.\n")
	b.WriteString("  + Câu hỏi liên quan NGHIỆP VỤ CHUYÊN DỤNG / QUY TRÌNH / TÀI LIỆU RIÊNG mà sir đã upload lên hệ thống (vd hỏi về nội dung một file cụ thể, quy chuẩn nội bộ, convention riêng của dự án, dữ liệu công ty) → gọi `rag.search` trước để kiểm tra có tài liệu phù hợp không.\n")
	...
	b.WriteString("  + Nếu không chắc câu hỏi có liên quan tài liệu đã upload hay không, ưu tiên gọi `web.search` trước; chỉ gọi thêm `rag.search` khi câu hỏi thực sự nhắc đến ngữ cảnh nội bộ/tài liệu riêng của sir.\n")
