...
func buildRegistries(cfg config.Config, store *memory.Store) (code, research, general *tools.Registry) {
	...
	code.Register(tools.NewShellToolWithTimeout(cfg.ShellAllowedCommands, time.Duration(cfg.ShellTimeout)*time.Second))
	...
	general.Register(tools.NewShellToolWithTimeout(cfg.ShellAllowedCommands, time.Duration(cfg.ShellTimeout)*time.Second))
